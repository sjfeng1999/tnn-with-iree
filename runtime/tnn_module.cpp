...
typedef struct iree_tnn_blob_t {
  // Ideally first; used to track the reference count of the object.
  iree_vm_ref_object_t ref_object;
  // Allocator the message was created from.
  // Ideally pools and nested allocators would be used to avoid needing to store
...
iree_status_t iree_tnn_blob_create(std::array<float, 5> value,
                                   iree_allocator_t allocator,
                                   iree_tnn_blob_t** out_blob){

  iree_tnn_blob_t* blob = NULL;
  iree_allocator_malloc(
      allocator, sizeof(*blob), (void**)&blob);

  blob->ref_object.counter = IREE_ATOMIC_VAR_INIT(1);
...
iree_status_t iree_tnn_module_register_types() {
  if (iree_custom_message_descriptor.type and iree_tnn_blob_descriptor.type) {
    return iree_ok_status();  // Already registered.
  }
  iree_custom_message_descriptor.type_name =
      iree_make_cstring_view("tnn.message");
...
class TnnModuleState final {
 public:
  explicit TnnModuleState(iree_allocator_t host_allocator,
  ...
  StatusOr<vm::ref<iree_tnn_blob_t>> Relu(vm::ref<iree_custom_message_t> message) {
    vm::ref<iree_tnn_blob_t> blob;

    std::array<float, 5> dummy{1, 2, 3, 4, 6};
    printf("tnn blob created");
    iree_tnn_blob_create(dummy, message->allocator, &blob);
...
static const vm::NativeFunction<TnnModuleState> kTnnModuleFunctions[] = {
    vm::MakeNativeFunction("buffer_to_message", &TnnModuleState::BufferToMessage),
    vm::MakeNativeFunction("message_to_buffer", &TnnModuleState::MessageToBuffer),
    vm::MakeNativeFunction("print", &TnnModuleState::Print),
    vm::MakeNativeFunction("reverse", &TnnModuleState::Reverse),
    vm::MakeNativeFunction("get_unique_message", &TnnModuleState::GetUniqueMessage),
...
class TnnModule final : public vm::NativeModule<TnnModuleState> {
 public:
  ...
  StatusOr<std::unique_ptr<TnnModuleState>> CreateState(
      iree_allocator_t host_allocator) override {
    auto state = std::make_unique<TnnModuleState>(
...
extern "C" iree_status_t iree_tnn_module_create(
  ...
  *out_module = NULL;
  auto module = std::make_unique<TnnModule>(
      "tnn", host_allocator,
      iree::span<const vm::NativeFunction<TnnModuleState>>(
          kTnnModuleFunctions));
