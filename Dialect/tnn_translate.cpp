...
#include "Dialect/init_dialect.h"
...
int main(int argc, char **argv) {
    ...
    // Register the Tnn dialect
    mlir::iree_compiler::registerTnnDialect(registry);
    ...
    if (splitInputFile) {
        if (failed(mlir::splitAndProcessBuffer(std::move(input), processBuffer, output->os())))
            return 1;
    } else {
